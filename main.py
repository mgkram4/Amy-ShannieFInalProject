...
# Add another font for dialog and leaderboard
try:
...
# Update the draw_messages function to check enemies directly
# Update the draw_messages function for better door message display
def draw_messages():
    if is_at_door():
...
def draw_npc_dialog():
    if npc_message and current_time - message_time < DIALOG_DISPLAY_TIME:
        ...
        if is_near_npc():
            ...
            screen.blit(hint_text, (dialog_x + DIALOG_WIDTH - hint_text.get_width() - 10, 
                                  dialog_y + DIALOG_HEIGHT - hint_text.get_height() - 5))
